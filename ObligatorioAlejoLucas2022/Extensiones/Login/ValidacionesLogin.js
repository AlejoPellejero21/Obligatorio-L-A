...
function formValidator(validations) {
  ...
  while (iterador < loginValidators.length && !failedValidation) {

    const value = document.querySelector(loginValidators[iterador].id);
    if (!loginValidators[iterador].fnValidate(value)) {
      failedValidation = loginValidators[iterador];
    }
    iterador++;
  }
